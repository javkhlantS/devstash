...
`updateItem(itemId, data)` in `actions/items.ts` following the `{ success, data, error }` return pattern. Validates input with Zod, gets session via `auth()`, validates ownership, calls query function.
